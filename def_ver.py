import requests
def get_trmsa_price(url):
    requestsfile = requests.get(url)
    requestsfile.encoding='utf-8'
    html_file = requestsfile.text
    start_pos = html_file.index('mainJsonString')+17
    end_pos = html_file.index('var mainXmlString')-3
    json_file = html_file[start_pos:end_pos]
    #↑獲取json檔案 ↓初始化變數
    date_list = []
    ...
    p_value = (json_file[json_file.index('spotHigh":',highpost_start_find)+10:json_file.index(',"futureClosingLow',highpost_start_find)])
    date = max_year+max_y_mon+max_y_m_day
    return(date,p_value)
print(get_trmsa_price('https://www.trmsa.org.tw/front/metal?qryMetal=al'))
